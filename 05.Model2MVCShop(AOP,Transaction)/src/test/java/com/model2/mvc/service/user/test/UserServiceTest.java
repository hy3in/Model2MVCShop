...
//			userService.addUser(user);
	 ...
	 @Test
	 public void InsertUserGrade() throws Exception{
		 System.out.println("InsertUserGrade 시작@@@@");
		 int totalPrice = 0;
		 User user = new User();
		 totalPrice = userService.getTotalPrice("user05");

		 if(totalPrice<10000) {
			 userService.InsertGrade("user05", 1);
		 }else if(totalPrice>=10000 && totalPrice<200000) {
			 userService.InsertGrade("user05", 2);
		 }else {
			 userService.InsertGrade("user05", 3);
		 }
	 }
}